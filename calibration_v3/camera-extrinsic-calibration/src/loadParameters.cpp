...
int vehicle_detector::select_correct_frame( Settings &s ){
    ...
    int start_search_frame = ( s.radar_trigger - 2 )*fps; // todo: change these values in final release
    int end_search_frame = ( s.radar_trigger + 2 )*fps;  // todo: change these values in final release
    //cap.set(CV_CAP_PROP_POS_FRAMES, start_frame_number);
    cv::namedWindow("vehicle", CV_WINDOW_NORMAL);
    //
    cv::Rect_<float> scaled_radar;
    cv::Rect_<float> candid_vehicle;
    float iou=-1;
    //
    cv::namedWindow("radarf", CV_WINDOW_FREERATIO);
    cv::Mat rfrm = cv::imread(s.radar_trigger_frame_path);
    int radar_w = rfrm.cols;
    int radar_h = rfrm.rows;

    scaled_radar.x = (std::abs(s.radar_min_position.x) + std::abs(s.radar_min_position.x)) * .5;
    scaled_radar.width = std::abs(s.radar_max_position.x) - std::abs(s.radar_min_position.x) ;
    scaled_radar.height = scaled_radar.width;
    scaled_radar.y = (std::abs(s.radar_min_position.y) + std::abs(s.radar_max_position.y)) * 0.5;


    cv::Point p1  (std::abs(s.radar_min_position.x) * radar_w, std::abs(s.radar_min_position.y) * radar_h);
    cv::Point p2  (std::abs(s.radar_max_position.x) * radar_w, std::abs(s.radar_max_position.y) * radar_h);
...
int vehicle_detector::collect_target_position( Settings &s){
    ...
    if (!s.testflag){
        ...
        select_correct_frame( s );
        cap.open(s.test_video_file_path);
        //init_frame_num = ( s.radar_trigger - 1 )*s.fps;
    }
    //cv::VideoCapture cap( _video_path );
    //if(!cap.isOpened() )
    //    std::cerr << "cannot read the video file \n";
    // cap.open( _video_path );

    // calculate start frame number
    int fps = cap.get(CV_CAP_PROP_FPS);
    int count = cap.get(CV_CAP_PROP_FRAME_COUNT);
    cap.set(CV_CAP_PROP_POS_FRAMES, init_frame_num);
    cv::namedWindow("vehicle", CV_WINDOW_NORMAL);
    ...
    while (cap.read(frame)){
        ...
        if(frame.empty())
        {
            ...
            cv::Rect_<float>  candid_vehicle;
            float iou=0.1;
            select_candid_vehicle( frame, target_vehicle.at(framenum - init_frame_num -1), candid_vehicle, iou);
            // todo: check if it is acceptable detection
            target_vehicle.push_back(candid_vehicle);
            frame_index.push_back(framenum);

//            frame.copyTo(max_frame); // for visualization
//            cv::resize(max_frame, max_frame, cv::Size(_network_width, _network_height));
//            int x1 = (target_vehicle.at(framenum - init_frame_num -1).x -target_vehicle.at(framenum - init_frame_num -1).width/2 )* _network_width;
//            int y1 = (target_vehicle.at(framenum - init_frame_num -1).y -target_vehicle.at(framenum - init_frame_num -1).height/2 )* _network_height;
//            int w1 = x1 + target_vehicle.at(framenum - init_frame_num -1).width * _network_width;
//            int h1 = y1 + target_vehicle.at(framenum - init_frame_num -1).height * _network_height;
//
...
//            cv::waitKey(0);
            ...
            if (target_vehicle.size()>frame_limit)
                break;
        }
