...
CCfg::CCfg()
{
	m_oFrmSz = cv::Size(420, 340);
	//std::strcpy(m_acInFrmPth, ".\\data\\frm.png");	// in Windows
	std::strcpy(m_acInFrmPth, "../data/BilkentParking.jpg");	// in Linux
													//std::strcpy(m_acOutCamParamPth, ".\\data\\camParam.txt");	// in Windows
	std::strcpy(m_acOutCamParamPth, "../data/camParam_cyber.txt");	// in Linux
	m_nRszFrmHei = 0;
	m_nLenUnit = 1000;
	m_bCalSelVanLnFlg = true;
...
void CCfg::ldCfgFl()
{
	...
	poCfgFl = std::fopen("../data/cfg_cyber.json", "r");	// in Linux
